...
class bag2hdf5BR:
    ...
    def __init__(self,data_loc='/media/imager/FlyDataD/FlyDB/Fly0012',file_folder = 'toConvert',file_folder_out='hdf5_converted'):
        ## Find bag file
        os.chdir(data_loc)
        for f in os.listdir(file_folder):
            if f.endswith('.bag'):
                bag_path = data_loc+'/'+file_folder+'/'+f
                bag_file_name = f  # bag_path[-26:]
                print ('running: ' +  bag_file_name)
                self.inbag = rosbag.Bag(bag_path)
                self.cv_bridge = CvBridge()
                ## Initialize hdf5 file
                if not os.path.exists(os.path.join(data_loc,file_folder_out)):
                    os.mkdir(os.path.join(data_loc,file_folder_out))
...
bag2hdf5BR()
os.chdir('/home/imager/catkin/src/exp_scripts/scripts/021_muscle_and_tendon/thad_utility_scripts') # change to script location
print 'Finished.'
