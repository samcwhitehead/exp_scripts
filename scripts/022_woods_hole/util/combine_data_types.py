...
def get_fly_path(fly, fly_db_path=FLY_DB_PATH, folder_str=FOLDER_STR):
    ...
    if isinstance(fly, int):
        ...
        file_suffix = '_' + fly
        file_suffix = os.path.splitext(file_suffix)[-1]  # just to make sure '.bag' isn't at the end
...
def my_interp(t, dat, resamp_t, interp_kind=INTERP_KIND):
    ...
    interp_f = interp1d(t, dat, kind=interp_kind)
...
def load_all_data(fly, var_dict=VAR_DICT, general_h5_str=GENERAL_H5_STR, signals_str=SIGNALS_STR,
    ...
    for side in ['left', 'right']:
        ...
        with h5py.File(signals_path, 'r') as h5f:
            for key in h5f.keys():
                load_dict['_'.join([side, 'muscle', key])] = h5f[key][:]
...
def combine_fly_data(fly, var_dict=VAR_DICT, general_h5_str=GENERAL_H5_STR, signals_str=SIGNALS_STR,
    ...
    for key in loaddata.keys():
        ...
        t = loaddata[key + '_t'] - t0  # NB: following Thad's code, I'm subtracting off intial kine cam time
        dat = loaddata[key]
        ...
        if isinstance(dat[0], basestring):
            ...
            interp_dict[key] = np.array([dat[ind] for ind in idx_interp], dtype='S256')
            continue
        else:
    ...
    if save_flag:
        ...
        interp_df.to_hdf(os.path.join(fly_path, save_name + ".hdf5"))
