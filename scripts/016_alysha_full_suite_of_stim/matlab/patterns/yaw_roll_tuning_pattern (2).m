...
%modififed 2/19/16 to cycle through yaw/roll axes in 30 deg increments
%% Define constants
% universe of points:
...
% ROLL_OFFSET_DEG = 30;
% rot_matrix =[cos(deg2rad(ROLL_OFFSET_DEG)),0,-sin(deg2rad(ROLL_OFFSET_DEG)) ; 0,1,0; sin(deg2rad(ROLL_OFFSET_DEG)),0,cos(deg2rad(ROLL_OFFSET_DEG))];
% 
...
for frameInd = 1:numFrames
    tThisFrame_sec = t_sec(frameInd);
    angleToRotate = tThisFrame_sec*ROT_RADPERSEC;
    %rot_matrix =[cos(deg2rad(ROLL_OFFSET_DEG)),0,-sin(deg2rad(ROLL_OFFSET_DEG)) ; 0,1,0; sin(deg2rad(ROLL_OFFSET_DEG)),0,cos(deg2rad(ROLL_OFFSET_DEG))];
    rot_matrix = [cos(angleToRotate)+.25*(1-cos(angleToRotate)), -sqrt(3)/2*sin(angleToRotate), sqrt(3)/4*(1-cos(angleToRotate)); 
                 sqrt(3)/2*sin(angleToRotate), cos(angleToRotate), -.5*sin(angleToRotate);
                 sqrt(3)/4*(1-cos(angleToRotate)), .5*sin(angleToRotate), cos(angleToRotate)+.75*(1-cos(angleToRotate))];
    xyzThisFrame_meters = xyz_meters * rot_matrix;
    distanceThisFrame_meters =  sqrt(sum(xyzThisFrame_meters.^2,2));
    inclinationThisFrame_rad = acos(xyzThisFrame_meters(:,3)./distanceThisFrame_meters);
    azimuthThisFrame_rad = atan2(xyzThisFrame_meters(:,2),xyzThisFrame_meters(:,1));
    for pointIndex = 1:numPoints 
...
for frameInd = 1:numFrames
    ...
    angleToRotate = tThisFrame_sec*ROT_RADPERSEC;
    %rot_matrix =[cos(deg2rad(ROLL_OFFSET_DEG)),0,-sin(deg2rad(ROLL_OFFSET_DEG)) ; 0,1,0; sin(deg2rad(ROLL_OFFSET_DEG)),0,cos(deg2rad(ROLL_OFFSET_DEG))];
    rot_matrix = [cos(angleToRotate)+.75*(1-cos(angleToRotate)), -0.5*sin(angleToRotate), sqrt(3)/4*(1-cos(angleToRotate)); 
                 0.5*sin(angleToRotate), cos(angleToRotate), -sqrt(3)/2*sin(angleToRotate);
                 sqrt(3)/4*(1-cos(angleToRotate)), sqrt(3)/2*sin(angleToRotate), cos(angleToRotate)+.25*(1-cos(angleToRotate))];
    xyzThisFrame_meters = xyz_meters * rot_matrix;
    distanceThisFrame_meters =  sqrt(sum(xyzThisFrame_meters.^2,2));
    inclinationThisFrame_rad = acos(xyzThisFrame_meters(:,3)./distanceThisFrame_meters);
    azimuthThisFrame_rad = atan2(xyzThisFrame_meters(:,2),xyzThisFrame_meters(:,1));
...
%% Calculate ROLL (about 30) frames opposite direction
ROLL_OFFSET_DEG = 30;
rot_matrix =[cos(deg2rad(ROLL_OFFSET_DEG)),0,-sin(deg2rad(ROLL_OFFSET_DEG)) ; 0,1,0; sin(deg2rad(ROLL_OFFSET_DEG)),0,cos(deg2rad(ROLL_OFFSET_DEG))];
...
allPats(:,:,:,9) = frames;
%% Calculate ROLL (about 60) frames opposite direction
ROLL_OFFSET_DEG = 60;
rot_matrix =[cos(deg2rad(ROLL_OFFSET_DEG)),0,-sin(deg2rad(ROLL_OFFSET_DEG)) ; 0,1,0; sin(deg2rad(ROLL_OFFSET_DEG)),0,cos(deg2rad(ROLL_OFFSET_DEG))];
...
str = [directory_name '\Pattern_yaw_roll_48P_bd'];
save(str, 'pattern');
