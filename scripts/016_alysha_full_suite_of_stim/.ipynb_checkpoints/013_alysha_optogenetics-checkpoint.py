...
if __name__ == '__main__':
    try:
        ...
        def exc_cl_blocks(block_name,gain_x,gain_y,bias_x,bias_y,ch=False):
            ...
            if ch:
                ctrl.set_ao(CHRIMSON_CHANNEL,CHRIMSON_VOLTS)
            else:
                ctrl.set_ao(CHRIMSON_CHANNEL,0)
            ctrl.start()
            time.sleep(3)
            ctrl.stop()
        ...
        def exc_ol_blocks(block_name,gain_x,gain_y,bias_x,bias_y,ch=False):
            pattern_name = 'Pattern_4x4_blocks_60.mat'
            blk_pub.publish(block_name)
            print block_name
        ...
        def exc_ol_stripe(block_name,gain_x,gain_y,bias_x,bias_y,ch=False):
        ...
        ctups = [c for c in itertools.product(([CL_GAIN_X]),(0,-4,4),(0,-8,8),([0]),([False,True]))]
        conditions = dict()
        s='cl_blocks, g_x=%s, g_y=%s, b_x=%s, b_y=%s, ch=%s'
        ...
        conditions[last_idx +0] = (exc_ol_blocks,'ol_blocks, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=1'%( 12, 0), ( 12,  0, 0, 0, True))
        conditions[last_idx +1] = (exc_ol_blocks,'ol_blocks, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=1'%(-12, 0), (-12,  0, 0, 0, True))
        conditions[last_idx +2] = (exc_ol_blocks,'ol_blocks, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=1'%( 0, 4),  ( 0,   4, 0, 0, True))
        conditions[last_idx +3] = (exc_ol_blocks,'ol_blocks, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=1'%( 0,-4),  ( 0,  -4, 0, 0, True))
        conditions[last_idx +4] = (exc_ol_blocks,'ol_blocks, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=1'%( 0, 0),  ( 0,   0, 0, 0, True))
        conditions[last_idx +5] = (exc_ol_stripe,'ol_stripe, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=1'%( 70, 0), ( 70,  0, 0, 0, True))
        conditions[last_idx +6] = (exc_ol_stripe,'ol_stripe, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=1'%(-70, 0), (-70,  0, 0, 0, True))

        conditions[last_idx +7] = (exc_ol_blocks,'ol_blocks, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=0'%( 12, 0), ( 12,  0, 0, 0, False))
        conditions[last_idx +8] = (exc_ol_blocks,'ol_blocks, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=0'%(-12, 0), (-12,  0, 0, 0, False))
        conditions[last_idx +9] = (exc_ol_blocks,'ol_blocks, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=0'%( 0, 4),  ( 0,   4, 0, 0, False))
        conditions[last_idx +10] = (exc_ol_blocks,'ol_blocks, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=0'%( 0,-4),  ( 0,  -4, 0, 0, False))
        conditions[last_idx +11] = (exc_ol_blocks,'ol_blocks, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=0'%( 0, 0),  ( 0,   0, 0, 0, False))
        conditions[last_idx +12] = (exc_ol_stripe,'ol_stripe, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=0'%( 70, 0), ( 70,  0, 0, 0, False))
        conditions[last_idx +13] = (exc_ol_stripe,'ol_stripe, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=0'%(-70, 0), (-70,  0, 0, 0, False))
        ###################################################################################
        ###################################################################################
