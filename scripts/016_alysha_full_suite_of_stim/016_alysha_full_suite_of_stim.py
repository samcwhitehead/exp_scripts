...
if __name__ == '__main__':
    try:
        ...
        def exc_cl_stripe(block_name,gain_x,gain_y,bias_x,bias_y,ch=0):
            pattern_name = 'Pattern_bar.mat'
            ...
            #ctrl.set_mode('xrate=ch0','yrate=0')
            #ctrl.send_gain_bias(gain_x = CL_GAIN_X, gain_y = 0, bias_x = 0,bias_y = 0)
            #ctrl.send_gain_bias(gain_x = gain_x, gain_y = 0, bias_x = 0,bias_y = 0)
            #exp_pub.publish('condition=baseline')
            #ctrl.start()
        ...
        def exc_yaw_right (block_name,gain_x,gain_y,bias_x,bias_y,ch=0):
            ...
            #ctrl.set_position(18,0)
            ctrl.set_mode('xrate=funcx','yrate=funcy')
            ctrl.send_gain_bias(gain_x = 18, gain_y = gain_y, bias_x = bias_x, bias_y = bias_y)
            exp_pub.publish('condition=test')
            ctrl.start()
            time.sleep(7)
            ctrl.stop()
        ...
        def exc_yaw_left (block_name,gain_x,gain_y,bias_x,bias_y,ch=0):
            ...
            #ctrl.set_position(18,0)
            ctrl.set_mode('xrate=funcx','yrate=funcy')
            ctrl.send_gain_bias(gain_x = 18, gain_y = 0, bias_x = bias_x, bias_y = bias_y)
            exp_pub.publish('condition=test')
            ctrl.start()
            time.sleep(7)
            #ch_pub.publish('set_a30 %s'%(ch))
        ...
        def exc_pitch_up (block_name,gain_x,gain_y,bias_x,bias_y,ch=0):
            ...
            #ctrl.set_position(18,0)
            ctrl.set_mode('xrate=funcx','yrate=funcy')
            ctrl.send_gain_bias(gain_x = 14, gain_y = 0, bias_x = bias_x, bias_y = bias_y)
            exp_pub.publish('condition=test')
            ctrl.start()
            time.sleep(7)
            #ch_pub.publish('set_a30 %s'%(ch))
        ...
        conditions[last_idx +0] = (exc_roll_clockwise,'roll_clockwise_actually_pitch, g_x=%s, g_y=%s, b_x=0, b_y=0, ch=0'%( 12, 0), ( 12,  0, 0, 0, 0))  #12
        ...
        exp_pub.publish('condition=posttrial_closed_loop;gain=%s'%(-1))
        time.sleep(10)
        ctrl.stop()
