...
def get_pattern_dict():
    pattern_dict = {'stripe'     : 'Pattern_bar.mat',
                    'stripe2'    : 'Pattern_2_stripe_48P_RC_IMS.mat',
                    ...
                    'loom_left'  : 'Pattern_loom_axis_0.mat',
                    'loom_back'  : 'Pattern_loom_axis_1.mat',
                    'loom_right' : 'Pattern_loom_axis_2.mat',
                    'loom_front' : 'Pattern_loom_axis_3.mat',
                    'loom_highcontrast_front' : 'Pattern_6_looming_dark.mat',
                    'loom_highcontrast_back' : 'Pattern_6_looming_dark.mat',
                    'loom_highcontrast_left' : 'Pattern_6_looming_dark.mat',
                    'loom_highcontrast_right' : 'Pattern_6_looming_dark.mat',
                    'loom_ripple_front' : 'Pattern_9_looming_ripple_IMS.mat',
                    'loom_ripple_back' : 'Pattern_9_looming_ripple_IMS.mat',
                    'loom_ripple_left' : 'Pattern_9_looming_ripple_IMS.mat',
                    'loom_ripple_right' : 'Pattern_9_looming_ripple_IMS.mat',
                    }
...
def exc_visual_stim(ctrl, block_name, duration, gain_x=0, gain_y=0, bias_x=0, bias_y=0, ch=0, x_init=None, pre_motion_duration=None, post_motion_duration=None, x_position_func='default'):
    ...
    if block_name_split[0]=='cl':
        ...
        # index in the "y" position (4th dim of pattern array)
        if 'loom' in block_name:
            if 'front' in block_name:
                y_idx = 4
            elif 'back' in block_name:
                y_idx = 1
            elif 'left' in block_name:
                y_idx = 2
            elif 'right' in block_name:
                y_idx = 5
            else:
                y_idx = 1
        ...
        # index in the "x" position (3rd dim of pattern array)   
        if not x_init:
            x_init = 0  # start value "x" position of pattern (3rd dim of pattery array) 
    ...
    ctrl.set_position_function_by_name('X', x_position_func)  
    time.sleep(0.005)
    
    # do a premotion pause?
    if pre_motion_duration:
        time.sleep(pre_motion_duration)
    ...
    if post_motion_duration:
        time.sleep(post_motion_duration)
# ------------------------------------------
