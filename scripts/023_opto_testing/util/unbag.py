...
########################### GENERAL CONSTANTS ##################################################
################################################################################################
FLY_DB_PATH = '/home/imager/nsb_data/FlyDB'
...
class BagToHDF5:
    ...
    def __init__(self, fly_db_path=FLY_DB_PATH, fly_path=None):
        ...
        self.cam_topic_names = ['/ca_camera_left/image_raw',
                                ...
                                '/side_camera_1/image_raw/compressed']
    ...
    def read_kinefly_data(self, topic_name):
        ...
        aux_intensity = [msg[1].aux.intensity for msg in self.inbag.read_messages(topics=topic_name)
                      if isinstance(msg[1].aux.intensity, float) ]
        ...
        with h5py.File(self.h5f, 'a') as h5f:
            ...
            h5f.create_dataset('flystate_lgradients', data=lgradients)
            ...
            h5f.create_dataset('flystate_auxintensity', data=aux_intensity)
