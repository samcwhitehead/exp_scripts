...
...WHILE varying the wingbeat phase at which images are captured 
(so we should get tendons at different wingstroke phases)
...
# the default value for shutter timing from teensy code
WB_TRIG_PERCENT_DEFAULT = 50
...
PHASE_VALS = [25, 33, 50, 66]
PHASE_LIST = np.ceil(NUM_REPS/len(PHASE_VALS))*list(PHASE_VALS)
...
if __name__ == '__main__':
    try:
        ...
        # -----------------------------------------------------
        # establish camera strober connection and set default
        css = CameraStroberSerial()  # create instance of CameraStroberSerial
        css.set_wb_trig_percent(WB_TRIG_PERCENT_DEFAULT)
        ...
        # set camera shutter timing back to default
        css.set_wb_trig_percent(WB_TRIG_PERCENT_DEFAULT)
